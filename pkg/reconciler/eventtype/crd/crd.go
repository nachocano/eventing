...
import (
	...
	"knative.dev/eventing/pkg/reconciler/eventtype/producer"
...
// Reconciler implements controller.Reconciler for Non-Source CRDs resources.
...
func (r *Reconciler) ReconcileKind(ctx context.Context, crd *v1.CustomResourceDefinition) pkgreconciler.Event {
	...
	// 	1. Resolve GVR and GVK from a particular Non-Source CRD (i.e., those not labeled with duck.knative.dev/source = "true")
	//  2. Dynamically create a controller for it, if not present already. Such controller is in charge of reconciling
	//     duckv1.Resource resources with that particular GVR..
	...
	err = r.reconcileController(ctx, gvr, gvk)
	if err != nil {
...
func (r *Reconciler) deleteController(ctx context.Context, gvr *schema.GroupVersionResource) {
	...
	if found {
		...
		if found {
			logging.FromContext(ctx).Infow("Stopping NonSource Duck Controller", zap.String("GVR", gvr.String()))
			rc.cancel()
...
func (r *Reconciler) reconcileController(ctx context.Context, gvr *schema.GroupVersionResource, gvk *schema.GroupVersionKind) error {
	r.lock.RLock()
	_, found := r.controllers[*gvr]
	r.lock.RUnlock()
	if found {
	...
	sdc := producer.NewController(*gvr, *gvk)
	if sdc == nil {
		logging.FromContext(ctx).Errorw("Resource Duck Controller is nil.", zap.String("GVR", gvr.String()), zap.String("GVK", gvk.String()))
		return nil
	}
	...
	logging.FromContext(ctx).Infow("Starting Resource Duck Controller", zap.String("GVR", gvr.String()), zap.String("GVK", gvk.String()))
	go func(c *controller.Impl) {
		if c != nil {
			if err := c.Run(controller.DefaultThreadsPerController, sdctx.Done()); err != nil {
				logging.FromContext(ctx).Errorw("Unable to start Resource Duck Controller", zap.String("GVR", gvr.String()), zap.String("GVK", gvk.String()))
			}
		}
	}(rc.controller)
	return nil
