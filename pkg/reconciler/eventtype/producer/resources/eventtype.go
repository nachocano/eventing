...
// EventTypeArgs are the arguments needed to create an EventType for a Resource.
type EventTypeArgs struct {
	Resource    *duckv1.Resource
	CeType      string
	CeSchema    *apis.URL
	Description string
}
...
func MakeEventType(args *EventTypeArgs) *v1beta1.EventType {
	...
	fixedName := fmt.Sprintf("%x", md5.Sum([]byte(args.CeType+args.CeSchema.String()+string(args.Resource.GetUID()))))
	return &v1beta1.EventType{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fixedName,
			Labels:    Labels(args.Resource.Name),
			Namespace: args.Resource.Namespace,
			OwnerReferences: []metav1.OwnerReference{{
				APIVersion:         args.Resource.APIVersion,
				Kind:               args.Resource.Kind,
				Name:               args.Resource.Name,
				UID:                args.Resource.UID,
