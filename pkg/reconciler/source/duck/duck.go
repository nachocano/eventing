...
type eventTypeEntry struct {
	...
	Extensions     []v1.Extension `json:"extensions,omitempty"`
...
func (r *Reconciler) makeEventTypes(ctx context.Context, src *duckv1.Source) ([]v1.EventType, error) {
	entries := make(map[string]eventTypeEntry)
	// Get the description, schema, extensions and sourceTemplate from the CRD, in case they are there.
	...
	eventTypes := make([]v1.EventType, 0)
	// TODO ugly, dedupe code.
	// if CloudEventAttributes were specified, then use this...
	if src.Status.CloudEventAttributes != nil {
		for _, attrib := range src.Status.CloudEventAttributes {
			...
			var schema, description, sourceTemplate string
			var extensions []v1.Extension
			if v, ok := entries[attrib.Type]; ok {
				schema = v.Schema
				description = v.Description
				sourceTemplate = v.SourceTemplate
		...
		// otherwise use the values in the annotation.
	} else {
		for _, entry := range entries {
			schemaURL, err := apis.ParseURL(entry.Schema)
			if err != nil {
				logging.FromContext(ctx).Warn("Failed to parse schema as a URL", zap.String("schema", entry.Schema), zap.Error(err))
				schemaURL = nil
			}
			eventType := resources.MakeEventType(&resources.EventTypeArgs{
				Source:         src,
...
func (r *Reconciler) computeDiff(current []v1.EventType, expected []v1.EventType) ([]v1.EventType, []v1.EventType) {
	toCreate := make([]v1.EventType, 0)
	toDelete := make([]v1.EventType, 0)
	currentMap := r.asMap(current, r.keyFromEventType)
	expectedMap := r.asMap(expected, r.keyFromEventType)

	// Iterate over the slices instead of the maps for predictable UT expectations.
	for _, e := range expected {
		if c, ok := currentMap[r.keyFromEventType(&e)]; !ok {
	...
	for _, c := range current {
		if _, ok := expectedMap[r.keyFromEventType(&c)]; !ok {
			toDelete = append(toDelete, c)
		}
	}
...
func (r *Reconciler) asMap(eventTypes []v1.EventType, keyFunc func(*v1.EventType) string) map[string]v1.EventType {
	eventTypesAsMap := make(map[string]v1.EventType)
...
func (r *Reconciler) keyFromEventType(eventType *v1.EventType) string {
	return fmt.Sprintf("%s_%s_%s", eventType.Spec.Type, eventType.Spec.Schema, r.crdName)
}
