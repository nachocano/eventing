...
// controller reconciling v1.EventType.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.EventType. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.EventType) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.EventType.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.EventType. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.EventType) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.EventType if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.EventType.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.EventType) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.EventType if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1.EventType.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1.EventType) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.EventType) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.EventType resources.
type reconcilerImpl struct {
	...
	Lister eventingv1.EventTypeLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister eventingv1.EventTypeLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.EventType, desired *v1.EventType) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.EventingV1().EventTypes(desired.Namespace)
		...
		updater := r.Client.EventingV1().EventTypes(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.EventType) (*v1.EventType, error) {

	getter := r.Lister.EventTypes(resource.Namespace)
	...
	patcher := r.Client.EventingV1().EventTypes(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.EventType) (*v1.EventType, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.EventType, reconcileEvent reconciler.Event) (*v1.EventType, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
