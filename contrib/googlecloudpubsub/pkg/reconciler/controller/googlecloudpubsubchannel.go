...
func NewController(
	...
	dispatcherDeploymentName string,
	dispatcherServiceName string,
	googlecloudpubsubchannelInformer messaginginformers.GoogleCloudPubSubChannelInformer,
	...
	googlecloudpubsubchannelInformer.Informer().AddEventHandler(reconciler.Handler(r.impl.Enqueue))
...
func (r *Reconciler) OnAdd(obj interface{}) {
	r.impl.GlobalResync(r.googlecloudpubsubchannelInformer)
}
...
// converge the two. It then updates the Status block of the GoogleCloudPubSubChannel resource
...
func (r *Reconciler) Reconcile(ctx context.Context, key string) error {
	...
	// Get the GoogleCloudPubSubChannel resource with this namespace/name.
	original, err := r.googlecloudpubsubchannelLister.GoogleCloudPubSubChannels(namespace).Get(name)
	if apierrs.IsNotFound(err) {
		// The resource may no longer exist, in which case we stop processing.
		logging.FromContext(ctx).Error("GoogleCloudPubSubChannel key in work queue no longer exists")
		return nil
	} else if err != nil {
		return err
	}
	...
	// Reconcile this copy of the GoogleCloudPubSubChannel and then write back any status updates regardless of
...
func (r *Reconciler) reconcile(ctx context.Context, gcpsc *v1alpha1.GoogleCloudPubSubChannel) error {
	gcpsc.Status.InitializeConditions()

	if gcpsc.DeletionTimestamp != nil {
