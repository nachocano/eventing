...
// reconciler reconciles Channels with the rabbitmq provisioner. It sets up hanging polling for
...
func (r *reconciler) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		logging.FromContext(ctx).Desugar().Error("Unable to Get Channel", zap.Error(err))
	...
	logging.FromContext(ctx).Desugar().Info("Reconciling Channel")

	// Modify a copy, not the original.
	c = c.DeepCopy()

	// requeue, reconcileErr := r.reconcile(logging.WithLogger(ctx, zap.Any("channel", c)), c, pcs)
	//if reconcileErr != nil {
...
func (r *reconciler) shouldReconcile(c *eventingv1alpha1.Channel) bool {
	if c.Spec.Provisioner != nil {
		return provisionerController.IsControlled(c.Spec.Provisioner)
	}
	return false
}
...
func (r *reconciler) reconcile(ctx context.Context, c *eventingv1alpha1.Channel) (bool, error) {
	...
	// running in the background polling the RabbitMq Subscription.

	//channelKey := key(c)

	if c.DeletionTimestamp != nil {
		// We use a finalizer to ensure we stop listening on the RabbitMq Subscriptions.
	...
	// before interacting with RabbitMq.
...
//}
//
//// stopAllSubscriptions stops listening to all RabbitMq Subscriptions for the given Channel.
...
//}
//
//// stopAllSubscriptionsUnderLock stops listening to all RabbitMq Subscriptions for the given
//// Channel.
...
//// polling the RabbitMq Subscriptions representing it. It also removes listeners from Subscribers
