...
// This is the main method for the RabbitMq Channel dispatcher. It handles all the data-plane
// activity for RabbitMq Channels. It receives all events being sent to any rabbitmq Channel
// (via the receiver below) and watches all RabbitMq Subscriptions (via the dispatcher below),
...
func main() {
	...
	// RabbitMq) and the dispatcher (takes messages in RabbitMq and sends them in cluster) in this
	// binary.

	_, runnables := receiver.New(logger.Desugar(), mgr.GetClient())
