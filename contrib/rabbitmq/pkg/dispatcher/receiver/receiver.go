...
// Receiver parses Cloud Events and sends them to RabbitMq.
...
func (r *Receiver) newMessageReceiver() *provisioners.MessageReceiver {
	return provisioners.NewMessageReceiver(r.sendEventToQueue, r.logger.Sugar())
}

// sendEventToQueue sends a message to the RabbitMq Queue backing the Channel.
func (r *Receiver) sendEventToQueue(channel provisioners.ChannelReference, message *provisioners.Message) error {
	r.logger.Debug("Received message")
	ctx := context.Background()

	_, err := r.getChannel(ctx, channel)
	if err != nil {
		logging.FromContext(ctx).Desugar().Info("Unable to get the Channel", zap.Error(err), zap.Any("channelRef", channel))
		return err
	}
