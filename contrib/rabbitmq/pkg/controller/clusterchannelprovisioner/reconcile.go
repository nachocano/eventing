...
const (
	// Name is the name of the RabbitMQ ClusterChannelProvisioner.
	Name = "rabbitmq"
)
...
func (r *reconciler) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if errors.IsNotFound(err) {
		logging.FromContext(ctx).Desugar().Info("Could not find ClusterChannelProvisioner", zap.Error(err))
		return reconcile.Result{}, nil
	}

	// Any other error should be retried in another reconciliation.
	if err != nil {
		logging.FromContext(ctx).Desugar().Error("Unable to Get ClusterChannelProvisioner", zap.Error(err))
		return reconcile.Result{}, err
	}

	// Does this Controller control this ClusterChannelProvisioner?
	if !shouldReconcile(ccp.Namespace, ccp.Name) {
		logging.FromContext(ctx).Desugar().Info("Not reconciling ClusterChannelProvisioner, it is not controlled by this Controller", zap.String("APIVersion", ccp.APIVersion), zap.String("Kind", ccp.Kind), zap.String("Namespace", ccp.Namespace), zap.String("name", ccp.Name))
		return reconcile.Result{}, nil
	}
	logging.FromContext(ctx).Desugar().Info("Reconciling ClusterChannelProvisioner.")
	...
	if reconcileErr != nil {
		logging.FromContext(ctx).Desugar().Info("Error reconciling ClusterChannelProvisioner", zap.Error(reconcileErr))
		// Note that we do not return the error here, because we want to update the Status
		// regardless of the error.
	}

	if err = util.UpdateClusterChannelProvisionerStatus(ctx, r.client, ccp); err != nil {
		logging.FromContext(ctx).Desugar().Info("Error updating ClusterChannelProvisioner Status", zap.Error(err))
		return reconcile.Result{}, err
	}
...
// IsControlled determines if the rabbitmq Channel Controller should control (and therefore
...
// ClusterChannelProvisioner. This Controller only handles rabbitmq channels.
