...
// allowing other processes to send data to them. It also creates the RabbitMq Queues (one per
// Channel) and RabbitMQ Subscriptions (one per Subscriber).
...
func (r *reconciler) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if errors.IsNotFound(err) {
		logging.FromContext(ctx).Desugar().Info("Could not find Channel", zap.Error(err))
	...
	if reconcileErr != nil {
		logging.FromContext(ctx).Desugar().Info("Error reconciling Channel", zap.Error(reconcileErr))
		// Note that we do not return the error here, because we want to update the Status
		// regardless of the error.
	}

	if err = util.UpdateChannel(ctx, r.client, c); err != nil {
		logging.FromContext(ctx).Desugar().Info("Error updating Channel Status", zap.Error(err))
		return reconcile.Result{}, err
	}
...
// Channel. This Controller only handles rabbitmq channels.
...
func (r *reconciler) reconcile(ctx context.Context, c *eventingv1alpha1.Channel) (bool, error) {
	...
	logging.FromContext(ctx).Desugar().Info("Reconciling RabbitMq Channel")

	// We are syncing four things:
	// 1. The K8s Service to talk to this Channel.
	// 2. The Istio VirtualService to talk to this Channel.
	// 3. The RabbitMq Queue (one for the Channel).
	// 4. The RabbitMq Subscriptions (one for each Subscriber of the Channel).
