...
const (
	...
	controllerAgentName = "rabbitmq-clusterchannelprovisioner-controller"
)

type RabbitMqProvisionerConfig struct {
	Url string
}

// ProvideController returns a Controller that represents the rabbitmq channel Provisioner. It
...
func ProvideController(mgr manager.Manager, logger *zap.Logger) (controller.Controller, error) {
	...
	// Setup a new controller to Reconcile ClusterChannelProvisioners that are rabbitmq channels.
	r := &reconciler{
